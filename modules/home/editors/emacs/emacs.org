...
And let's highlight all the things
...
:NOTE:
This config uses =after-init= hook instead of a deferred as the =doom-modeline-mode= gets activated; but it doesn't show the modeline. For whatever reason, not an issue with the hook
:END:
