...
let
  cfg = config.my.nixos.vyprvpn;
in
{
  options.my.nixos.vyprvpn = {
    enable = lib.mkEnableOption "Whether to enable VyprVPN settings";
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.openvpn.servers.vyprvpn = {
      ...
      config = ''
        ...
        auth-user-pass ${config.sops.secrets.vyprvpn-auth-user-pass.path}
      '';
    };
  };
}
