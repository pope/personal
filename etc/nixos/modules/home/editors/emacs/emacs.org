...
The minibuffer package, which is built into Emacs, allows for all sort of
completion commands, from code completions to searching through variables,
functions, and files.
...
This section is /heavily/ influenced by [[https://sophiebos.io/posts/beautifying-emacs-org-mode/][Beautifying Emacs Org Mode]] post. It
adds some extra font goodness when using the GUI.
...
:NOTE:
=noweb= is a feature of =org-tangle= that does the replacement for values
inside of the backets, referenced by name. Each reference has a =:cache no= so
that the expression isn't evaluated - and thus substituted right in.
:END:


** Fonts

So the first thing to do, set the default variable fonts. This is part of the
=:custom-face= part of =use-package= so that they can still be customized per
device.

I did try to set =fixed-pitch= to inherit from =default=, but that didn't seem
to work. So if customizing one, customize the other to match.
...
After that, we can set custom font sizes for each org heading level and
document title.
...
If =org-indent= mode is used, we need to inherit from fixed-pitch as to not
have funky alignment issues with mixed font widths.
...
=olivetti= will center the org document in the window. And =org-modern-mode=
to apply some interesting styles to things.
...
Now when =org-mode= is activated, we can enable those two libraries, but there
are some modes that need to be disabled.

- =diff-hl-margin-mode=  :: funky in both visuals and in performance, though it
  does conflict some with org-modern-mode visuals right now.
- =visual-line-mode= :: I keep the auto-fill-mode on, so text fits in the
  view. However, some block code can start wrapping and it's a sign I should
  format that code a little better. Though, I may rethink this with the use of
  olivetti.
- =indent-bars-mode= :: Disabling this doesn't really feel like it's working,
  but it does add a little bit of visual noise.
